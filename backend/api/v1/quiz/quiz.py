...
@quiz_router.get("/")
async def get_all_quizzes(controller: QuizController = Depends(Factory().get_quiz_controller)) -> QuizOut:
    return await controller.repository.get_all()

@quiz_router.get("/{id}")
async def get_quiz(id: int, controller: QuizController = Depends(Factory().get_quiz_controller)) -> list[QuizOut]:
...
async def create_quiz(quiz: QuizSchema, controller: QuizController = Depends(Factory().get_quiz_controller)) -> QuizOut:
